...
APK reverse + MobSF. Apoya M8 cuando cliente tiene app mobile.

## Tools expuestos

| Tool | Funcion (breve) |
|---|---|
| `apktool` | (ver `tools/apktool_tool.py::TOOL` - descripcion en docstring) |
| `mobsf` | (ver `tools/mobsf_tool.py::TOOL` - descripcion en docstring) |
...
`Dockerfile` presente - imagen base con runtime + requirements.txt. Construir con:
    ...
    docker build -t fulkro-mcp-mobile backend/mcp_servers/mobile/
