...
Aircrack + wifite - audita redes WiFi cliente. Apoya mp.com.* en sedes fisicas.

## Tools expuestos

| Tool | Funcion (breve) |
|---|---|
| `aircrack_attack` | (ver `tools/aircrack_attack_tool.py::TOOL` - descripcion en docstring) |
| `aircrack_scan` | (ver `tools/aircrack_scan_tool.py::TOOL` - descripcion en docstring) |
| `wifite` | (ver `tools/wifite_tool.py::TOOL` - descripcion en docstring) |
...
`Dockerfile` presente - imagen base con runtime + requirements.txt. Construir con:
    ...
    docker build -t fulkro-mcp-wireless backend/mcp_servers/wireless/
