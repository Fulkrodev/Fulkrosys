...
Hashcat + John + wordlist gen. Apoya auditoria fortaleza credenciales op.acc.5/6.

## Tools expuestos

| Tool | Funcion (breve) |
|---|---|
| `ad_password_check` | (ver `tools/ad_password_check_tool.py::TOOL` - descripcion en docstring) |
| `cewl` | (ver `tools/cewl_tool.py::TOOL` - descripcion en docstring) |
| `hashcat` | (ver `tools/hashcat_tool.py::TOOL` - descripcion en docstring) |
| `john` | (ver `tools/john_tool.py::TOOL` - descripcion en docstring) |
...
`Dockerfile` presente - imagen base con runtime + requirements.txt. Construir con:
    ...
    docker build -t fulkro-mcp-cracking backend/mcp_servers/cracking/
