...
Tests seguridad APIs: param discovery, auth bypass, GraphQL. Apoya M8 MEDIA+ (servicios expuestos).

## Tools expuestos

| Tool | Funcion (breve) |
|---|---|
| `arjun` | (ver `tools/arjun_tool.py::TOOL` - descripcion en docstring) |
| `auth_bypass` | (ver `tools/auth_bypass_tool.py::TOOL` - descripcion en docstring) |
| `graphql` | (ver `tools/graphql_tool.py::TOOL` - descripcion en docstring) |
| `kiterunner` | (ver `tools/kiterunner_tool.py::TOOL` - descripcion en docstring) |
...
`Dockerfile` presente - imagen base con runtime + requirements.txt. Construir con:
    ...
    docker build -t fulkro-mcp-apisec backend/mcp_servers/apisec/
