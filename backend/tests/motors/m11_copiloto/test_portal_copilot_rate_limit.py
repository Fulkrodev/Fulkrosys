...
async def _seed_cliente_llm_interactions(db: AsyncSession, count: int) -> None:
    """Inserta ``count`` interacciones copiloto_cliente hoy (UTC) para empujar
    el contador diario de mensajes por encima del cap (CLIENTE_CAPS=100)."""
...
async def test_quick_actions_hard_blocked_returns_429(
    ...
    await _seed_cliente_llm_interactions(db, 120)  # > 100 cap diario
...
async def test_chat_hard_blocked_returns_429_before_llm(
    ...
    cli, _stub = authed_client_user
    await _seed_cliente_llm_interactions(db, 120)
