...
async def test_schedule_biannual_audit_creates_entry(db: AsyncSession):
    ...
    assert entry.next_audit_due == conformity_date + timedelta(
        days=BIANNUAL_PERIOD_DAYS
    )
...
async def test_schedule_biannual_idempotent(db: AsyncSession):
    ...
    assert biannual[0].next_audit_due == date(2026, 8, 1) + timedelta(
        days=BIANNUAL_PERIOD_DAYS
    )
