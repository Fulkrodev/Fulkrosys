...
def test_task_client(gen):
    tid = uuid.uuid4()
    url = gen.task(tid)
    assert url == f"{BASE}/client-portal/tasks?task={tid}"
...
def test_task_admin(gen):
    ...
    assert url == f"{BASE}/admin/projects/{pid}/workflow?task={tid}"
...
def test_evidence_client(gen):
    ...
    assert url == f"{BASE}/client-portal/evidencias?evidence={eid}"
...
def test_evidence_admin(gen):
    ...
    assert url == f"{BASE}/admin/projects/{pid}/evidence?evidence={eid}"
...
def test_audit_client(gen):
    aid = uuid.uuid4()
    url = gen.audit(aid)
    assert url == f"{BASE}/client-portal/certificacion?audit={aid}"
...
def test_audit_admin(gen):
    ...
    assert url == f"{BASE}/admin/projects/{pid}/audit?audit={aid}"
...
def test_dashboard_admin(gen):
    assert gen.dashboard_admin() == f"{BASE}/admin/dashboard"
...
def test_account_notifications(gen):
    ...
    assert gen.notifications() == (
        f"{BASE}/client-portal/settings/notifications"
    )
...
def test_magic_link(gen):
    ...
    assert url == f"{BASE}/ml/consume?token=abc123token"
...
def test_client_user_admin(gen):
    ...
    assert gen.client_user_admin(cid, uid) == (
        f"{BASE}/admin/clients/{cid}?user={uid}"
    )
    pid = uuid.uuid4()
    assert gen.client_user_admin(cid, uid, project_id=pid) == (
        f"{BASE}/admin/projects/{pid}/users?user={uid}"
    )
