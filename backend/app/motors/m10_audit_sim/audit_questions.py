...
AUDIT_QUESTIONS: dict[str, dict[str, Any]] = {
    ...
    "op.pl.5": {
        "pregunta": "¿Se emplean componentes (productos/equipos) con certificación de seguridad reconocida (Common Criteria, CCN-STIC/CPSTIC) acorde al nivel exigido?",
        "criterio": "Selección de componentes certificados según CPSTIC/CCN cuando aplica + inventario con su certificación",
        "evidencia_tipos": ["componentes_certificados", "cpstic"],
        "documento_esperado": None,
        "familia": "op.pl",
    ...
    "op.exp.9": {
        "pregunta": "¿Existe un registro de la gestión de incidentes de seguridad (detección, clasificación, respuesta y cierre)?",
        "criterio": "Registro de incidentes con fecha, clasificación, acciones, responsable y cierre + trazabilidad",
        "evidencia_tipos": ["registro_incidentes", "gestion_incidentes"],
        "documento_esperado": None,
        "familia": "op.exp",
    ...
    # ═══════════════════ CONTINUIDAD (op.cont) ═══════════════════
    "op.cont.1": {
        "pregunta": "¿Existe un análisis de impacto (BIA) que identifique los procesos/servicios críticos y defina RTO/RPO?",
        "criterio": "BIA documentado con procesos críticos + RTO/RPO + valoración del impacto de la interrupción",
    ...
    "mp.if.3": {
        "pregunta": "¿Los locales donde se ubican los sistemas cuentan con acondicionamiento adecuado (climatización, control de temperatura y humedad)?",
        "criterio": "Climatización + control de temperatura y humedad + mantenimiento de las condiciones ambientales",
        "evidencia_tipos": ["acondicionamiento_locales", "climatizacion"],
        "documento_esperado": None,
        "familia": "mp.if",
    ...
    "mp.per.2": {
        "pregunta": "¿El personal conoce y acepta sus deberes y obligaciones de seguridad (acuerdos de confidencialidad, RSEG, régimen disciplinario)?",
        "criterio": "Deberes/obligaciones definidos + acuerdos de confidencialidad firmados + comunicación documentada al personal",
        "evidencia_tipos": ["deberes_obligaciones", "acuerdo_confidencialidad"],
        "documento_esperado": None,
        "familia": "mp.per",
    ...
    "mp.eq.2": {
        "pregunta": "¿Los puestos de trabajo se bloquean automáticamente tras un periodo de inactividad y exigen reautenticación?",
        "criterio": "Bloqueo de sesión por inactividad (GPO/MDM) + reautenticación + cierre de sesión tras periodo prolongado",
        "evidencia_tipos": ["bloqueo_puesto", "politica_bloqueo_sesion"],
        "documento_esperado": None,
        "familia": "mp.eq",
    ...
    "mp.eq.3": {
        "pregunta": "¿Cómo se protegen los dispositivos portátiles? ¿Están cifrados y gestionados (MDM/EDR)?",
        "criterio": "Cifrado de disco (BitLocker/FileVault) + MDM/EDR + política de dispositivos móviles + evidencia de cobertura",
    ...
    "mp.info.3": {
        "pregunta": "¿Se emplea firma electrónica en los documentos/registros que lo requieren, con el nivel adecuado?",
        "criterio": "Sistema de firma electrónica + política de firma + registros de uso + nivel acorde al riesgo",
    ...
    "mp.info.4": {
        "pregunta": "¿Se aplican sellos de tiempo a la información/registros que lo requieren?",
        "criterio": "Sello de tiempo cualificado + autoridad de sellado de confianza + uso en registros relevantes",
    ...
    "mp.info.5": {
        "pregunta": "¿Se eliminan los metadatos y la información oculta de los documentos antes de su distribución/publicación?",
        "criterio": "Procedimiento de limpieza de metadatos + herramientas + aplicación previa a la distribución",
        "evidencia_tipos": ["limpieza_documentos", "metadatos"],
        "documento_esperado": None,
        "familia": "mp.info",
    ...
    # ═══════════════════ SERVICIOS (mp.s) ═══════════════════
    "mp.s.1": {
        "pregunta": "¿Cómo se protege el correo electrónico (anti-spam/anti-phishing, SPF/DKIM/DMARC, cifrado en tránsito)?",
        "criterio": "Anti-spam/anti-malware + SPF/DKIM/DMARC + cifrado en tránsito + concienciación anti-phishing",
        "evidencia_tipos": ["proteccion_correo", "spf_dkim_dmarc"],
        "documento_esperado": None,
        "familia": "mp.s",
    ...
    "mp.s.2": {
        "pregunta": "¿Cómo se protegen los servicios y aplicaciones web (HTTPS/TLS, WAF, gestión de vulnerabilidades, OWASP)?",
        "criterio": "HTTPS/TLS + WAF + gestión de vulnerabilidades web + buenas prácticas OWASP + pruebas periódicas",
        "evidencia_tipos": ["proteccion_web", "waf", "https"],
        "documento_esperado": None,
        "familia": "mp.s",
        "nombre": "Medios alternativos",
        "aplica": ["ALTA"],
