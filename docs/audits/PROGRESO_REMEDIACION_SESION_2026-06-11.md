...
### SIGUIENTE PASO (orden): F5 (R15→R20→R26) → F6 (R23) → 3 simulaciones + auditor + docs
**Hallazgos empíricos para la próxima tanda (F5/F6 · scope mayor · cada item es un generador real):**
...
- **R26** (certificado ENAC descargable + NC estructuradas · ~3-5h): m27 tiene `distintivo_generator.py` + `ines_generator.py`. Falta: descarga del certificado/distintivo ENAC + modelo estructurado de No Conformidades (NC mayor/menor/observación + estado + plan de acción) que alimente el informe de auditoría (E-050/E-700).
