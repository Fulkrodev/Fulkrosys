# F2.1 — PROCEDIMIENTOS CRÍTICOS DEL SGSI ENS — TEXTO OPERATIVO REAL EN ESPAÑOL (E-200 a E-218)

**Plan 100/100 FULKRO — Bloque 3 de plantillas reales**
**Versión:** 1.0 — 9 de abril de 2026
**Continuación de:** F1_1 + F1_2 (políticas críticas E-100 a E-108)
...
|---|---|---|---|
| E-200 | Procedimiento de Análisis y Gestión de Riesgos | E-100 | op.pl.1, op.pl.2 |
| E-203 | Procedimiento de Gestión de la Información Documentada | E-100 | org.2, org.3 |
| E-204 | Procedimiento de Gestión de Incidentes de Seguridad | E-103 | op.exp.7 |
| E-205 | Procedimiento de Gestión de Cuentas y Accesos | E-102 | op.acc.4 |
| E-217 | Procedimiento de Evaluación y Seguimiento de Proveedores | E-107 | op.ext.1, op.ext.2 |
| E-218 | Procedimiento de Gestión de Vulnerabilidades y Parches | E-100, E-105 | op.exp.4 |

---

# DOCUMENTO E-200 — PROCEDIMIENTO DE ANÁLISIS Y GESTIÓN DE RIESGOS
...
# DOCUMENTO E-203 — PROCEDIMIENTO DE GESTIÓN DE LA INFORMACIÓN DOCUMENTADA DEL SGSI
...
a) **Documentación normativa:** la Política de Seguridad ({{ proyecto.codigo_documento_base }}-100), las políticas específicas (E-101 a E-126), los procedimientos (E-200 a E-2XX) y las instrucciones técnicas asociadas.
...
| Procedimientos operativos | {{ proyecto.codigo_documento_base }}-200 a -299 | Comité de Seguridad | Anual |
...
**Es el procedimiento operativo que ejecuta la política E-103.** Mientras la política dice qué hay que notificar y a quién, este procedimiento dice exactamente qué hace cada persona desde el minuto cero hasta el cierre del incidente.
...
3. Realizar una **clasificación preliminar** del incidente conforme a la escala del apartado 4 de la Política {{ proyecto.codigo_documento_base }}-103 (niveles BAJO, MEDIO, ALTO, MUY ALTO o CRÍTICO).
...
7. **Iniciar la cadena de custodia** de las evidencias conforme al procedimiento {{ proyecto.codigo_documento_base }}-228, registrando:
...
11. Decidir si procede activar el **Plan de Continuidad del Servicio** ({{ proyecto.codigo_documento_base }}-104).
...
15. **Notificación al CCN-CERT vía LUCIA**, cuando proceda, en los plazos establecidos en el apartado 6.1 de la Política {{ proyecto.codigo_documento_base }}-103:
...
27. **Cerrar formalmente el incidente** en el Sistema de Gestión de Incidentes, manteniendo accesible toda la documentación durante el periodo de conservación establecido en el procedimiento {{ proyecto.codigo_documento_base }}-203.
...
# DOCUMENTO E-205 — PROCEDIMIENTO DE GESTIÓN DE CUENTAS Y ACCESOS

**Es el procedimiento operativo de la Política E-102.** Define quién pide, quién autoriza, quién provisiona, quién revisa y quién revoca cada acceso del sistema. El auditor pide ver el flujo completo en al menos 3 casos reales (alta, modificación, baja).
...
clasificacion: "INTERNA"
propietario: "{{ responsables.responsable_seguridad.cargo }}"
politica_madre: "{{ proyecto.codigo_documento_base }}-102"
---
...
Establecer el flujo operativo para la solicitud, autorización, provisión, modificación, revisión periódica y revocación de cuentas de usuario y derechos de acceso a los sistemas de información de {{ cliente.razon_social }}, en desarrollo de la Política de Control de Acceso ({{ proyecto.codigo_documento_base }}-102).
...
- Respeta las reglas de segregación de funciones del apartado 7 de la Política {{ proyecto.codigo_documento_base }}-102.
...
- Cumplimiento de la política de contraseñas del apartado 5.3 de {{ proyecto.codigo_documento_base }}-102.
...
10. Notificar al usuario la disponibilidad de su cuenta, sus credenciales iniciales y las normas de uso aceptable ({{ proyecto.codigo_documento_base }}-106).
...
**Periodicidad:** según el apartado 6.4 de la Política {{ proyecto.codigo_documento_base }}-102:
...
22. El Responsable del Sistema procede a la revocación en los plazos máximos del apartado 6.5 de la Política {{ proyecto.codigo_documento_base }}-102:
...
**Es el procedimiento operativo de la Política E-107.** Define exactamente cómo se evalúa a un proveedor antes de contratar, durante la prestación y al finalizar la relación.
...
1. Antes del inicio de cualquier negociación contractual con un proveedor de nivel **MEDIO** o superior conforme al apartado 3 de la Política {{ proyecto.codigo_documento_base }}-107, se le remitirá el **Cuestionario de Evaluación de Seguridad** del Anexo I, que contiene preguntas sobre:
...
4. Se elabora un **análisis de riesgos específico** de la relación, conforme a la metodología MAGERIT v3 ({{ proyecto.codigo_documento_base }}-200), considerando:
...
8. El contrato a suscribir incorporará las cláusulas mínimas de seguridad establecidas en el apartado 5.3 de la Política {{ proyecto.codigo_documento_base }}-107.
...
- Se asignan los accesos necesarios al proveedor según el procedimiento {{ proyecto.codigo_documento_base }}-205.
...
- Se le entrega copia de las políticas E-106 (Uso Aceptable) y E-108 (Clasificación) en lo aplicable.
...
- Revocación de todos los accesos del proveedor a sistemas, redes e instalaciones de la Entidad, conforme al procedimiento {{ proyecto.codigo_documento_base }}-205.
...
23. Se elabora el **Informe de Cierre de Proveedor** y se archiva el expediente conforme al procedimiento {{ proyecto.codigo_documento_base }}-203.
...
# DOCUMENTO E-218 — PROCEDIMIENTO DE GESTIÓN DE VULNERABILIDADES Y PARCHES
...
clasificacion: "INTERNA"
propietario: "{{ responsables.responsable_seguridad.cargo }}"
politica_madre: "{{ proyecto.codigo_documento_base }}-100, {{ proyecto.codigo_documento_base }}-105"
---
...
9. El despliegue se realiza conforme al **procedimiento de gestión de cambios** ({{ proyecto.codigo_documento_base }}-206), con la autorización correspondiente al riesgo del cambio.
...
|---|---|---|
| E-200 Análisis y Gestión de Riesgos | E-100 | ✅ Completo |
| E-203 Gestión de la Información Documentada | E-100 | ✅ Completo |
| E-204 Gestión de Incidentes | E-103 | ✅ Completo |
| E-205 Gestión de Cuentas y Accesos | E-102 | ✅ Completo |
| E-217 Evaluación y Seguimiento de Proveedores | E-107 | ✅ Completo |
| E-218 Gestión de Vulnerabilidades y Parches | E-100, E-105 | ✅ Completo |
...
| E-207 | Procedimiento de Concienciación y Formación | E-100, E-106 | mp.per.3, mp.per.4 |
...
| E-234 | Procedimiento de Auditoría Interna del SGSI | E-100 | (transversal ENS art. 31) |
...
2. **Diagramas de flujo:** los procedimientos E-204 y E-205 contienen flujos operativos que se beneficiarían de un diagrama visual. Claude Code puede generarlos automáticamente con `mermaid` o `graphviz` y embedarlos en el `.docx` final como imagen PNG.
...
|---|---|---|
| **org** | ~90% | E-100, E-101, E-203 |
| **op.pl** | 100% | E-100, E-200 |
| **op.acc** | 100% | E-102, E-205 |
| **op.exp** | ~70% (resto en F2.2 con E-206, E-219, E-228) | E-218, parcial E-204 |
| **op.ext** | 100% | E-107, E-217 |
| **op.cont** | ~50% (resto en F2.2 con E-210) | E-104 |
| **mp.if** | 0% (será una política E-110 secundaria) | — |
| **mp.per** | ~30% (resto en F2.2 con E-207) | E-100, E-106 |
| **mp.eq** | ~80% | E-106, E-218 |
| **mp.com** | 100% | E-105 |
| **mp.si** | 100% | E-105 |
| **mp.sw** | ~40% (mayor parte en políticas secundarias) | E-218 |
| **mp.info** | 100% | E-105, E-108 |
